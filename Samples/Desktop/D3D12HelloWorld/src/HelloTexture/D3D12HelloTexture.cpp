...
void D3D12HelloTexture::LoadAssets()
{
	...
	ThrowIfFailed(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocator2.Get(), m_pipelineState.Get(), IID_PPV_ARGS(&m_commandList2)));
	...
	ThrowIfFailed(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocatorb.Get(), nullptr, IID_PPV_ARGS(&m_commandListb)));
	ThrowIfFailed(m_commandListb->Close());
	ThrowIfFailed(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocatora.Get(), nullptr, IID_PPV_ARGS(&m_commandLista)));
...
void D3D12HelloTexture::OnRender()
{
	...
	ThrowIfFailed(m_commandLista->Reset(m_commandAllocatora.Get(), nullptr));
...
void D3D12HelloTexture::PopulateCommandList()
{
	...
	ThrowIfFailed(m_commandAllocator2->Reset());
	ThrowIfFailed(m_commandAllocator3->Reset());

	// However, when ExecuteCommandList() is called on a particular command 
	// list, that command list can then be reset at any time and must be before 
	// re-recording.
	ThrowIfFailed(m_commandList->Reset(m_commandAllocator.Get(), m_pipelineState.Get()));
	ThrowIfFailed(m_commandList2->Reset(m_commandAllocator2.Get(), m_pipelineState.Get()));
	ThrowIfFailed(m_commandList3->Reset(m_commandAllocator3.Get(), m_pipelineState.Get()));

	// Set necessary state.
	m_commandList->SetGraphicsRootSignature(m_rootSignature.Get());
	m_commandList2->SetGraphicsRootSignature(m_rootSignature.Get());
	m_commandList3->SetGraphicsRootSignature(m_rootSignature.Get());

	ID3D12DescriptorHeap* ppHeaps[] = { m_srvHeap.Get() };
	m_commandList->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);
	m_commandList2->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);
	m_commandList3->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);

	m_commandList->SetGraphicsRootDescriptorTable(0, m_srvHeap->GetGPUDescriptorHandleForHeapStart());
	m_commandList2->SetGraphicsRootDescriptorTable(0, m_srvHeap->GetGPUDescriptorHandleForHeapStart());
	m_commandList3->SetGraphicsRootDescriptorTable(0, m_srvHeap->GetGPUDescriptorHandleForHeapStart());

	m_commandList->RSSetViewports(1, &m_viewport);
	m_commandList2->RSSetViewports(1, &m_viewport);
	m_commandList3->RSSetViewports(1, &m_viewport);

	m_commandList->RSSetScissorRects(1, &m_scissorRect);
	m_commandList2->RSSetScissorRects(1, &m_scissorRect);
	m_commandList3->RSSetScissorRects(1, &m_scissorRect);
	...
	m_commandList2->OMSetRenderTargets(1, &rtvHandle, FALSE, nullptr);
	m_commandList3->OMSetRenderTargets(1, &rtvHandle, FALSE, nullptr);
	...
	m_commandList2->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	m_commandList3->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	m_commandList->IASetVertexBuffers(0, 1, &m_vertexBufferView);
	m_commandList2->IASetVertexBuffers(0, 1, &m_vertexBufferView);
	m_commandList3->IASetVertexBuffers(0, 1, &m_vertexBufferView);

	m_commandList2->DrawInstanced(3, 1, 0, 0);

	// Indicate that the back buffer will now be used to present.
	m_commandList3->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargets[m_frameIndex].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT));
